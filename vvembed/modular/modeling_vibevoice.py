...
class VibeVoicePreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):               
        ...
        use_external_llm = getattr(self.config, 'use_external_llm', False)
    ...
    def _load_state_dict_into_model(self, state_dict, *args, **kwargs):
        # Check if we should use external LLM
        use_external_llm = getattr(self.config, 'use_external_llm', False)
        
        # Filter out LLM weights if using external LLM
        if use_external_llm:
            # Remove keys related to language model
        ...
        return super()._load_state_dict_into_model(state_dict, *args, **kwargs)        
...
class VibeVoiceModel(VibeVoicePreTrainedModel):
    def __init__(self, config):
        ...
        # Check if we should use external LLM (from config or fallback)
        self.use_external_llm = getattr(config, 'use_external_llm', False)
        
        # Initialize Qwen2 model for language modeling
        # Conditionally initialize language model
        lm_config = config.decoder_config 
        if not config.use_external_llm:            
            self.language_model = AutoModel.from_config(lm_config)
        else:
            self.language_model = None  # Will be set externally       
...
class VibeVoiceForConditionalGeneration(VibeVoicePreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        print("before VibeVoiceModel(config)")
        self.model = VibeVoiceModel(config)
         # Conditionally create LM head
        if not config.use_external_llm:
