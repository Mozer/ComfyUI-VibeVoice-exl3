...
class VibeVoiceMultipleSpeakersNode(BaseVibeVoiceNode):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "diffusion_steps": ("INT", {"default": 15, "min": 5, "max": 50, "step": 1, "tooltip": "Number of denoising steps. More steps = better quality but slower. Default: 20"}),
            ...
            "optional": {
                ...
                "negative_llm_steps_to_cache": ("INT", {"default": 4, "min": 0, "max": 100, "tooltip": "Cache n steps of negative conditioning for speed-up. 0 - cache turned off for best quality; 10 - for speed (may cause noise on long text)"}),
                ...
                "split_by_newline": ("BOOLEAN", {"default": True, "tooltip": "Split long text into chunks by each speaker"}),
            }
        }
    ...
    def generate_speech(self, text: str = "", model: str = "VibeVoice-7B-Preview",
                       ...
                       streaming = False, streaming_buffer = 10,
                       exllama_model = "None", negative_llm_steps_to_cache: int = 0, 
                       increase_cfg: bool=False, split_by_newline: bool=False):
        ...
        try:
            ...
            for i, speaker_num in enumerate(speakers_in_text):
                ...
                all_voice_samples.append(voice_sample)
            
            # Ensure voice_samples count matches detected speakers
            if len(all_voice_samples) != len(speakers_in_text):
                logger.error(f"Mismatch: {len(speakers_in_text)} speakers but {len(all_voice_samples)} voice samples!")
                raise Exception(f"Voice sample count mismatch: expected {len(speakers_in_text)}, got {len(all_voice_samples)}")
